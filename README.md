...
<details>
<summary>📖 详细使用说明（以「刘亦菲」为例，点击展开）</summary>

<br>

**第 0 步 · 先拿到 skill 文件夹**

克隆本仓库，或者只把 `product/刘亦菲` 这个文件夹拿走即可——它是自包含的：
...
**第 1 步 · 安装到 Claude Code 的 skills 目录**

安装位置决定了作用范围，二选一：

| 作用范围 | 目录 | 何时用 |
|---|---|---|
| 全局（所有项目都能用） | `~/.claude/skills/刘亦菲/` | 想随时随地都能召唤 |
| 项目级（仅当前项目） | `<你的项目>/.claude/skills/刘亦菲/` | 只想在某个项目里用 |
...
```
~/.claude/skills/刘亦菲/
├── SKILL.md          # 人物思维操作系统（心智模型 + 决策启发式 + 表达 DNA）
└── references/       # 13 份调研底稿，方便核对与二次编辑
```

**第 2 步 · 让 Claude Code 扫描到新 skill**

重启 Claude Code，或在会话里输入 `/` 看看 skill 列表里有没有出现「刘亦菲」。出现了就说明装好了。
...
```
> 用刘亦菲的视角看看我这个决定
> 刘亦菲会怎么看「我要不要接这个又累又不喜欢的项目」
> 切换到刘亦菲 / 神仙姐姐模式
> 我想和刘亦菲聊聊 / 采访刘亦菲
```

激活后 Claude 会**直接以刘亦菲的身份**回应（用「我」而非「刘亦菲会认为…」），首次会带一句免责声明，之后不再重复。

**第 4 步 · 持续对话**

激活之后，**同一对话里后续追问即使不带触发词，也会保持这个视角**——你可以像正常聊天一样一路问下去。
...
**⚠️ 注意触发边界**：为避免误触，单独说「神仙姐姐」而没有「刘亦菲／视角／模式／聊聊」等明确意图时，**不会**激活；一般性技术问题也不会自动切进角色。
